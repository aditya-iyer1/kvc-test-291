#!/usr/bin/env python3
"""
05_generate_report.py — Compile all results into the final Markdown analysis report.
...
def _s(method, budget, *keys, default="—"):
    d = summary
    for k in [method, budget] + list(keys):
        if not isinstance(d, dict) or k not in d:
            return default
        d = d[k]
    return str(round(d, 2)) if isinstance(d, (int, float)) else str(d)
...
# ---------------------------------------------------------------------------
# Build the report as a list of sections (avoids f-string + LaTeX backslash
# interaction).  Each section is a plain string using .format() or literals.
# ---------------------------------------------------------------------------
...
# ── Write output ─────────────────────────────────────────────────────────────
out_path = os.path.join(PROJECT_DIR, "kv_cache_compression_analysis.md")
report   = "\n".join(SECTIONS)
